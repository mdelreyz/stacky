...
def _infer_preferences(conversation: list[WizardTurn], user_message: str) -> dict:
...
def _build_follow_up_question(inferred: dict) -> str | None:
...
def _normalize_recommended_items(items: list[dict] | None, catalog: CatalogSnapshot) -> list[dict]:
    ...
    catalog_by_id = {entry["id"]: entry for entry in _catalog_entries(catalog)}
    catalog_by_name = {entry["name"].lower(): entry for entry in _catalog_entries(catalog)}
    ...
    for item in items:
        ...
        if catalog_match is None:
            continue
        if catalog_match["id"] in seen_catalog_ids:
            continue
...
def _build_completion_message(payload: dict) -> str:
...
def _parse_completion_payload(assistant_text: str, inferred: dict, catalog: CatalogSnapshot) -> dict | None:
    try:
...
def run_wizard_turn(
    *,
    ...
    inferred = _infer_preferences(conversation, user_message)
    ...
    try:
        ...
        fallback_result = _static_fallback(conversation, user_message, catalog)
        if isinstance(fallback_result, dict):
            completion_payload = fallback_result
            assistant_text = _build_completion_message(fallback_result)
        else:
            assistant_text = fallback_result

    if completion_payload is None:
        completion_payload = _parse_completion_payload(assistant_text, inferred, catalog)
        if completion_payload is not None:
            assistant_text = _build_completion_message(completion_payload)
...
def _static_fallback(conversation: list[WizardTurn], user_message: str, catalog: CatalogSnapshot) -> str | dict:
    """Provide a question-by-question fallback when Claude is unavailable."""
    inferred = _infer_preferences(conversation, user_message)
    next_question = _build_follow_up_question(inferred)
