...
async def list_user_therapies(
    ...
    base_query = select(UserTherapy).where(UserTherapy.user_id == current_user.id)
    if active_only:
        base_query = base_query.where(UserTherapy.is_active.is_(True))

    count_result = await session.execute(select(func.count()).select_from(base_query.subquery()))
    total = count_result.scalar_one()

    offset = (page - 1) * page_size
    result = await session.execute(base_query.order_by(UserTherapy.created_at.desc()).offset(offset).limit(page_size))
    user_therapies = list(result.scalars().all())

    return PaginatedResponse(
        items=[await serialize_user_therapy(user_therapy) for user_therapy in user_therapies],
        total=total,
        page=page,
        page_size=page_size,
