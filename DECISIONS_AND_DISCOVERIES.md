...
## 18. Essential supplement scoring is now intentionally sparse and universal-first (2026-04-11)

The supplement detail score panel no longer treats `essential` as a generic proxy for evidence plus safety. `apps/mobile/lib/supplement-score.ts` now normalizes overlapping signal labels, penalizes niche or protocol-specific entries, supports the catalog's existing `traditional` and `speculative` evidence tiers, and rewards only broad core-system coverage plus nutrient-repletion patterns that plausibly fit an almost-everybody default stack. Tuned against the current local catalog, the distribution now lands at roughly 12 supplements above 90, 33 between 50 and 90, and the rest below 50, which matches the product intent better than the previous near-universal 90+ inflation.

## 19. Legacy take-window values must be normalized at the backend boundary (2026-04-11)

The post-auth "Failed to load daily plan" bug turned out to be a backend enum-compatibility regression, not an auth problem. Persisted regimen rows in the live SQLite DB still carried older `take_window` strings such as `evening_with_food` and `with_meals`, and SQLAlchemy started raising enum decode errors when daily-plan and tracking reads touched those rows. The backend now owns compatibility: `apps/api/app/models/enums.py` maps legacy labels onto the canonical window set, `apps/api/app/services/take_window_compat.py` normalizes persisted rows plus adherence snapshots on startup, and `apps/api/app/services/recommendation_application.py` canonicalizes recommendation-apply windows before writing them back into enum-backed tables.
